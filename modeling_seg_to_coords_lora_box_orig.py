...
Date: 2025-03-13 14:38:18
LastEditors: git config user.name && git config user.email
LastEditTime: 2025-03-13 14:40:12
FilePath: \毕设\doc-codeedit\doc-code\modeling_seg_to_coords_lora_box copy.py
Description: 原始文件
...
class LisaGSVAForCausalLM(LlavaLlamaForCausalLM):
    ...
    def model_forward(
        ...
        **kwargs,
        ...
        for i in range(len(pot_token_offset) - 1):
            # 处理点
        ...
        pred_masks = []
        pred_ious = []
        for i in range(len(predicted_points_list_resized)):
        ...
        # 总损失
        loss = ce_loss + mask_loss + center_loss + box_loss
