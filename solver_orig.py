...
from data.doctamper_dataset import DocTamperDataset
...
def train_one_epoch2(train_loader, model_engine, epoch, train_iter, args, logger, input_dict=None, current_step=0):
    ...
    progress = ProgressMeter(
        ...
        [batch_time, losses, ce_losses, mask_losses, mask_bce_losses, 
         mask_dice_losses, center_losses, quality_meter, box_losses],
...
def eval_gres(val_loader, model_engine, epoch, args, logger):
    ...
    accuracy_meter = AverageMeter("Accuracy", ":6.3f", Summary.AVERAGE)
    mean_iou_meter = AverageMeter("Mean IoU", ":6.3f", Summary.AVERAGE)
    ...
    f1_meter.all_reduce()
    accuracy_meter.all_reduce()
    mean_iou_meter.all_reduce()
    iou_meter.all_reduce()  # 新增 IoU 的 all_reduce

    g_iou = g_iou_meter.avg[1]  # 计算平均广义IoU
    c_iou = (inter_meter.sum / (union_meter.sum + 1e-10))[1]  # 计算类别为1（目标）的交并比
    precision_avg = precision_meter.avg
    recall_avg = recall_meter.avg
    f1_avg = f1_meter.avg
    accuracy_avg = accuracy_meter.avg  # 获取平均准确率
    mean_iou = mean_iou_meter.avg  # 获取平均IoU
    iou_avg = iou_meter.avg  # 获取平均 IoU

    logger.info(
        f"[{epoch + 1:d}] {val_loader.dataset.val} "
        f"miou: {mean_iou:.4f}, iou: {iou_avg:.4f}, f1: {f1_avg:.4f}, "
        f"giou: {g_iou:.4f}, ciou: {c_iou:.4f}, "
        f"precision: {precision_avg:.4f}, recall: {recall_avg:.4f}, "
    )
    return g_iou, c_iou, f1_avg, precision_avg, recall_avg, accuracy_avg, mean_iou, iou_avg
